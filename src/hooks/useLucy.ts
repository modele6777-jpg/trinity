...
import { callGemini, generateSpeech, checkQuota } from '../services/geminiService';
...
export function useLucy() {
  ...
  const startQuickReading = async (activeForm: any) => {
    ...
    try {
      ...
      const greeting = await callGemini(sysPrompt, `질문자가 퀵 리딩을 요청했습니다. 고민("${activeForm.concern}")에 대해 타로 카드(${drawn[0].kr})와 사주/별자리 기운을 섞어 아주 짧고 명쾌한 통찰을 건네주세요.`);
      const { messages: parsed, emotion } = parseChat(greeting, 'Lucy');
      if (emotion) setCurrentEmotion(emotion);
